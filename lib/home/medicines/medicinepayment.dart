...
class medipay extends StatefulWidget {
  final Map<String, dynamic> medicine;

  medipay({required this.medicine});

  @override
  _MedicinePaymentPageState createState() => _MedicinePaymentPageState();
}

class _MedicinePaymentPageState extends State<medipay> {
  ...
  void _placeOrder(BuildContext context) async {
    ...
    try {
      ...
      if (user != null) {
        await _firestore.collection('medicineDetails').add({
          'userId': user.uid,
          'medicineId': widget.medicine['id'],  // Make sure 'id' is present in medicine data
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content: Text('Your order will be delivered in 24 hours.'),
                actions: [
                  TextButton(
                    onPressed: () {
        ...
        child: Column(
          ...
          children: [
            widget.medicine['image'] != null
                ? Image.network(widget.medicine['image'], width: double.infinity, height: 200, fit: BoxFit.cover)
                : Container(height: 200, color: Colors.grey),
            SizedBox(height: 16),
            Text('Name: ${widget.medicine['name'] ?? 'No name'}', style: TextStyle(fontSize: 20)),
            Text('Details: ${widget.medicine['details'] ?? 'No details'}', style: TextStyle(fontSize: 16)),
            SizedBox(height: 16),
            Divider(),
